...
class IntegratedADML {
  ...
  async installGithubRepo(fullName) {
    try {
      ...
      const releaseResponse = await fetch(`https://api.github.com/repos/${encodeURIComponent(fullName)}/releases/latest`, { headers });
      let downloadUrl = "";
      let sourceName = `${fullName.replace(/[^a-z0-9._-]/gi, "-")}.zip`;
      if (releaseResponse.ok) {
        const release = await releaseResponse.json();
        const asset = release.assets?.find(item => item.name.toLowerCase().endsWith(".zip"));
        if (asset) { downloadUrl = asset.browser_download_url; sourceName = asset.name; }
      }
      if (!downloadUrl) {
        const repoResponse = await fetch(`https://api.github.com/repos/${fullName}`, { headers });
        if (!repoResponse.ok) throw new Error(`Repository HTTP ${repoResponse.status}`);

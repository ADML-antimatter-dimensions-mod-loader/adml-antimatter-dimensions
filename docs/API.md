...
This document provides the complete API reference for the Antimatter Dimensions MOD Loader (ADML) v2.3 runtime [1]. All APIs are accessible inside `plugin.js` via the `api` instance passed to `constructor(api)` or `onload(api)`.
